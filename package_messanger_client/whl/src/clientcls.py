...
from storagesqlite import Storage, PATH, History
    ...
    def __init__(cls, name_class, bases, dict_class):
        ...
        for key, value in dict_class.items():
            if isinstance(value, type(socket(AF_INET, SOCK_STREAM))):
                raise Exception(
                    "Должно отсутствовать создание сокетов на уровне класса"
                )
            a = dis.code_info(value)
            if "accept" in a:
                raise Exception(
                    "Такой функции у сокета, как 'accept' не должна быть в классе"
                )
            elif "listen" in a:
                raise Exception(
                    "Такой функции у сокета,как 'listen' не должна быть в классе"
                )
...
class Client(metaclass=ClientVerifier):
    ...
    def add_message(self, message):
        ...
        base_data = Storage(
            PATH, "history_message", {"id_send": int(id_user), "message": message}
        )
        base_data.add_base()
    ...
    def add_contact(self, cons):
        ...
        id_user = user_history.id_user
        base_data = Storage(
            PATH, "list_contacts", {"id_send": int(id_user), "list_con": cons}
        )
        base_data.add_base()
    ...
    def cli_start(self):
        ...
        msg_text = (
            "Введите, что вы хотите сделать (П/Отправить сообщение пользователю,Г/Отправить группе, "
    ...
    def cli_send_g(self):
        ...
        msg_a_text = (
            "Введите номер группы с #100 до #999 с которым хотите начать беседу: "
        )
    ...
    def cli_add_g(self):
        ...
        msg_a_text = (
            "Введите номер группы от #100 которую хотите создать или подключится: "
        )
        msg_a = self.validate_get(msg_a_text)
