...
PATH = f"sqlite:////home/{os.getlogin()}/Documents/Messenger/sqlite3.db"
...
class Storage:
                ...
                mapper(
                    ...
                    properties={
                        "addresses": relationship(
                            History, backref="user", order_by=history.c.id
                        ),
                        "adr": relationship(
                            Contacts, backref="user", order_by=contacts.c.id
                        ),
                    },
                )
    ...
    def add_base(self):
        if self.table == "users":
            self.con_base()
            base_a = User(
                login=self.args_dict["login"],
                password=self.args_dict["password"],
                information=self.args_dict["information"],
            )
        elif self.table == "histories_users":
            self.con_base()
            base_a = History(
                id_user=self.args_dict["id_user"],
            ...
            self.con_base()
            base_a = Contacts(
                id_send=self.args_dict["id_send"], id_recv=self.args_dict["id_recv"]
            )
        elif self.table == "history_message":
            self.con_base()
            base_a = History_Message(
                id_send=self.args_dict["id_send"], message=self.args_dict["message"]
            )
        elif self.table == "list_contacts":
            self.con_base()
            base_a = List_Contact(
                id_send=self.args_dict["id_send"],
                list_con=str(self.args_dict["list_con"]),
            )
        else:
            base_a = ""
